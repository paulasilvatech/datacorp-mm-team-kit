...
Dono do caminho do código até algo que roda. No workshop, quem garante que o modo de execução criado pelo time funciona em qualquer máquina, que o CI checa o que importa, e que o Terraform descreve a topologia-alvo no Azure mesmo que não seja aplicado no dia.
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| 1. Arqueologia         | Valida ferramentas locais e planeja o que será necessário para execução local do protótipo.                          | Ambiente de trabalho validado            |
...
- **Azure / Terraform MCP** se habilitado no VS Code.
- **GitHub Spec-Kit** — `/speckit.taskstoissues` e os gates operacionais passam por você.
- Docker e Compose — você cria apenas o que o protótipo do time realmente precisa.
...
- O modo de execução local sobe aplicação + banco em menos de 60s quando o protótipo existir.
...
- Deixa setup local ambíguo e o time perde 1 hora no início.
...
3. **(Chat)** _"O ambiente local demora 3 minutos para subir. Analise os arquivos criados pelo time e proponha 3 otimizações."_

## Se travar (defaults de emergência)

- Ambiente local não sobe? Checklist: (1) Docker Desktop rodando, se houver containers? (2) Portas 5432/8080/3000 livres? (3) variáveis de ambiente definidas? (4) logs do backend/frontend indicam causa-raiz?
...
- Critério: "Build reproduzível. Qualquer máquina do time roda o ambiente local documentado em menos de 60s."
