...
| 9 | A routine "add my current IP to the NSG" apply produced a plan of **14 to destroy**, including both VMs and the attachment of the disk holding the loaded Adabas database. Applying it would have silently wiped a working lab to change one firewall rule. | This tenant mutates resources at create time: it rewrote both OS disks and the data disk from `Premium_LRS` to `Standard_LRS`, created both subnets with `defaultOutboundAccess=false`, and enrolled both VMs in Azure Update Manager (`patch_mode = AutomaticByPlatform`). `main.tf` still declared the values it had *asked* for, so every later plan tried to force them back — and `default_outbound_access_enabled` and `os_disk.storage_account_type` are both replace-forcing, which cascaded through the subnets into the VMs. Declaring the nicer value never produced it; it only armed a destructive plan. | Guardrail: [`infra/adabas-natural-lab/tests/test_platform_drift.py`](../../infra/adabas-natural-lab/tests/test_platform_drift.py) asserts `main.tf` declares the platform-applied values, so restoring `Premium_LRS` or dropping `default_outbound_access_enabled = false` fails the suite. Review point: read `Plan:` before every apply — for an existing lab it must say `0 to destroy`, and `deploy-local.sh plan` prints it. | Plan went from `14 to add, 5 to change, 14 to destroy` to `4 to add, 2 to change, 0 to destroy` once the config adopted the applied values. |
| 10 | The Windows DDM workstation was deployed with no public IP (correctly, since the tenant strips public RDP rules) into a subnet with `defaultOutboundAccess=false`, leaving it with **no outbound internet at all** — while its only purpose is downloading the NaturalONE installer. | Azure retired default outbound access for new deployments, so a VM now needs an explicit egress path. The design removed the public IP for security without adding one, and the gap was invisible because the VM boots, RDP works over Bastion, and nothing fails until someone tries to reach the internet. | Guardrail: `test_should_give_the_workstation_an_outbound_path` in the same file requires a NAT gateway and its subnet association, and `test_should_keep_the_nat_address_off_the_nic` keeps that address off the workstation NIC so the fix cannot regress into exposed RDP. | Workstation egress IP `20.14.151.87` matches `sifap-lab-pip-natgw` exactly, confirming the NAT gateway is the path. |
